defmodule SymphonyElixir.Config.Schema do
  ...
  defmodule Tracker do
    ...
    defmodule ImageInputs do
      ...
      embedded_schema do
        ...
        field(:max_images, :integer, default: 3)
        field(:allowed_hosts, {:array, :string}, default: ["uploads.linear.app"])
        field(:allow_http, :boolean, default: false)
      end
      ...
      def changeset(schema, attrs) do
        ...
        schema
        |> cast(attrs, [:enabled, :max_images, :allowed_hosts, :allow_http], empty_values: [])
        |> validate_number(:max_images, greater_than: 0)
      end
      ...
      defp normalize_attrs(attrs) when is_map(attrs) do
        ...
        |> maybe_update(
          "allowed_hosts",
          &SymphonyElixir.Config.Schema.normalize_csv_string_list(&1, trim?: true, downcase?: true)
        )
      end
      ...
      defp maybe_update(map, key, fun) when is_map(map) do
    ...
    embedded_schema do
      ...
      embeds_one(:image_inputs, ImageInputs, on_replace: :update, defaults_to_struct: true)
    end
    ...
    def changeset(schema, attrs) do
      ...
      |> cast_embed(:image_inputs, with: &ImageInputs.changeset/2)
      |> validate_length(:active_states, min: 1)
      |> validate_length(:terminal_states, min: 1)
    end

    defp normalize_attrs(attrs) when is_map(attrs) do
      attrs
      |> normalize_map_keys()
      |> maybe_update("active_states", &SymphonyElixir.Config.Schema.normalize_csv_string_list/1)
      |> maybe_update("terminal_states", &SymphonyElixir.Config.Schema.normalize_csv_string_list/1)
    end
  ...
  defmodule Agent do
    ...
    def changeset(schema, attrs) do
      ...
      |> validate_number(:max_continuations, greater_than: 0)
  ...
  defp normalize_csv_string_item(value, opts) do
    value
    |> to_string()
    |> maybe_trim(Keyword.get(opts, :trim?, true))
    |> maybe_downcase(Keyword.get(opts, :downcase?, false))
    |> case do
      "" -> nil
