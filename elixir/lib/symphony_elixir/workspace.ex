defmodule SymphonyElixir.Workspace do
    ...
    try do
      ...
      with {:ok, workspace} <- workspace_path_for_issue(safe_id, worker_host),
        ...
        case maybe_run_after_create_hook(workspace, issue_context, created?, worker_host) do
          ...
          {:error, reason} = error ->
            cleanup_failed_new_workspace(workspace, issue_context, created?, worker_host, reason)
            error
        end
      end
    rescue
  ...
  defp cleanup_failed_new_workspace(
         workspace,
         issue_context,
         true,
         worker_host,
         {:workspace_hook_timeout, "after_create", _timeout_ms}
       ) do
  ...
  defp cleanup_failed_new_workspace(
         ...
         true,
         worker_host,
         {:workspace_hook_failed, "after_create", _status, _output}
       ) do
    cleanup_failed_workspace(workspace, issue_context, worker_host)
  end
  ...
  defp cleanup_failed_workspace(workspace, issue_context, worker_host) when is_binary(worker_host) do
    ...
    case run_remote_command(worker_host, script, Config.settings!().hooks.timeout_ms) do
      ...
      {:ok, {output, status}} ->
        Logger.warning(
          "Failed to clean workspace after after_create failure #{issue_log_context(issue_context)} workspace=#{workspace} worker_host=#{worker_host} status=#{status} output=#{inspect(sanitize_hook_output_for_log(output))}"
        )
